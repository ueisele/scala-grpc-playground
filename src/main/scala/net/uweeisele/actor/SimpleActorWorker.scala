...
object SimpleActorWorker {
  def apply[T](initialBehaviour: Behaviour[T])(implicit ec: ExecutionContext): SimpleActorWorker[T] = {
    val worker = new SimpleActorWorker[T](initialBehaviour, new LinkedBlockingQueue[T]())
    ec.execute(worker)
    worker
  }
}

class SimpleActorWorker[T](initialBehaviour: Behaviour[T], queue: BlockingQueue[T]) extends Runnable {
