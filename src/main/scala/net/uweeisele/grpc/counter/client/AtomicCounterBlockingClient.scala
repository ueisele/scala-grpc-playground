    ...
    val channel = NettyChannelBuilder
      ...
      .intercept(new ClientInterceptor {
        override def interceptCall[ReqT, RespT](method: MethodDescriptor[ReqT, RespT], callOptions: CallOptions, next: Channel): ClientCall[ReqT, RespT] = {
          logger.info(s"InterceptCall => thread: ${Thread.currentThread().getName} method: ${method.getFullMethodName}")
            ...
            override def start(responseListener: ClientCall.Listener[RespT], headers: Metadata): Unit = {
              super.start(new ClientCall.Listener[RespT]() {
                ...
                override def onMessage(message: RespT): Unit = {
                  logger.info(s"InterceptResponse => thread: ${Thread.currentThread().getName} method: ${method.getFullMethodName}")
                  responseListener.onMessage(message)
                }

                override def onClose(status: Status, trailers: Metadata): Unit = {
                  logger.info(s"onClose => thread: ${Thread.currentThread().getName}")
